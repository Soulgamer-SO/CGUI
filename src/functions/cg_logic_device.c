...
bool cg_create_logic_device(cg_info_t *p_info, VkDevice *p_vk_logic_device) {
	p_info->logic_device.queue_family_count = 0;
	PFN_vkGetPhysicalDeviceQueueFamilyProperties get_physical_device_queue_family_properties = nullptr;
	get_physical_device_queue_family_properties = (PFN_vkGetPhysicalDeviceQueueFamilyProperties)p_info->library.vk_get_instance_proc_addr(
		p_info->instance.vk_instance, "vkGetPhysicalDeviceQueueFamilyProperties");
	...
	get_physical_device_queue_family_properties(
		p_info->physical_device.physical_device, &p_info->logic_device.queue_family_count, nullptr);
	if (p_info->logic_device.queue_family_count == 0) {
	...
	if (p_info->logic_device.queue_family_array == nullptr) {
	...
	get_physical_device_queue_family_properties(
		p_info->physical_device.physical_device, &p_info->logic_device.queue_family_count, p_info->logic_device.queue_family_array);
	if (p_info->logic_device.queue_family_count == 0) {
		PRINT_ERROR("get queue_familiy_array fail!\n");
		return false;
	}

	p_info->logic_device.queue_family_index = 0;
	p_info->logic_device.queue_priority_array_count = 16;
	p_info->logic_device.queue_priority_array_index = 0;
	p_info->logic_device.queue_priority_array = (float *)cg_alloc_memory(
		p_info->p_memory_pool,
		p_info->logic_device.queue_priority_array_count * sizeof(float));
	if (p_info->logic_device.queue_priority_array != nullptr) {
		PRINT_LOG("alloc memory success!\n");
		for (uint32_t i = 0; i < p_info->logic_device.queue_priority_array_count; i++) {
			p_info->logic_device.queue_priority_array[i] = 0.001f * (i + 1);
			PRINT_LOG("queue_priority_array[%d] = %f;\n", i, p_info->logic_device.queue_priority_array[i]);
		}
	}

#ifdef DEBUG
	// 打印所有队列信息
	char **queue_info = cg_alloc_memory(p_info->p_memory_pool, 5 * sizeof(char *));
	...
	for (p_info->logic_device.queue_family_index = 0;
		...
		if (p_info->logic_device.queue_family_array[p_info->logic_device.queue_family_index].queueCount > 0) {
			...
			if (p_info->logic_device.queue_family_array[p_info->logic_device.queue_family_index].queueFlags & VK_QUEUE_COMPUTE_BIT) {
				queue_info[1] = " VK_QUEUE_COMPUTE_BIT";
			} else {
				queue_info[1] = "";
			}
			if (p_info->logic_device.queue_family_array[p_info->logic_device.queue_family_index].queueFlags & VK_QUEUE_TRANSFER_BIT) {
				queue_info[2] = " VK_QUEUE_TRANSFER_BIT";
			} else {
				queue_info[2] = "";
			}
			if (p_info->logic_device.queue_family_array[p_info->logic_device.queue_family_index].queueFlags & VK_QUEUE_SPARSE_BINDING_BIT) {
				queue_info[3] = " VK_QUEUE_SPARSE_BINDING_BIT";
			} else {
				queue_info[3] = "";
			}
			if (p_info->logic_device.queue_family_array[p_info->logic_device.queue_family_index].queueFlags & VK_QUEUE_PROTECTED_BIT) {
	...
	VkDeviceCreateInfo device_create_info = {
		...
		.enabledExtensionCount = p_info->physical_device.enabled_physical_device_extensions_count,
		.ppEnabledExtensionNames = (const char *const *)&p_info->physical_device.enabled_physical_device_extension_array[0],
		.pEnabledFeatures = &p_info->physical_device.device_feature_array};

	// create logic device
	PFN_vkCreateDevice create_device = nullptr;
	create_device = (PFN_vkCreateDevice)p_info->library.vk_get_instance_proc_addr(
		p_info->instance.vk_instance, "vkCreateDevice");
	if (create_device == nullptr) {
		PRINT_ERROR("load vkCreateDevice device fail!\n");
		return false;
	}

	p_info->library.vk_result = create_device(
		p_info->physical_device.physical_device, &device_create_info,
		nullptr, p_vk_logic_device);
	if (p_info->library.vk_result != VK_SUCCESS || p_vk_logic_device == VK_NULL_HANDLE) {
		PRINT_ERROR("create vulkan logic device fail!\n");
		return false;
	}

#ifdef DEBUG
	for (uint32_t i = 0; i < p_info->physical_device.enabled_physical_device_extensions_count; i++) {
		PRINT_LOG("enabled_physical_device_extension_array[%d] = %s;\n", i, p_info->physical_device.enabled_physical_device_extension_array[i]);
	}
	PRINT_LOG("%d个物理设备扩展已经全部启用!\n", p_info->physical_device.enabled_physical_device_extensions_count);
	...
	get_device_queue = (PFN_vkGetDeviceQueue)p_info->library.vk_get_device_proc_addr(
