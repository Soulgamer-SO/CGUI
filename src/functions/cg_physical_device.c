...
bool cg_enumerate_physical_device(cg_info_t *p_info, uint32_t *p_physical_device_count, VkPhysicalDevice *available_physical_device_array) {
	...
	enumerate_physical_devices = (PFN_vkEnumeratePhysicalDevices)p_info->library.vk_get_instance_proc_addr(p_info->instance.vk_instance, "vkEnumeratePhysicalDevices");
	if (enumerate_physical_devices == nullptr) {
		PRINT_ERROR("load vkEnumeratePhysicalDevices fail!\n");
		return false;
	}
	p_info->library.vk_result = enumerate_physical_devices(p_info->instance.vk_instance, p_physical_device_count, available_physical_device_array);
	if (p_info->library.vk_result != VK_SUCCESS || *p_physical_device_count == 0) {
...
bool cg_select_physical_device(cg_info_t *p_info, uint32_t *p_physical_device_count, VkPhysicalDevice *available_physical_device_array, VkPhysicalDevice *p_physical_device) {
	// 初始化物理设备(选择默认显卡并初始)
	p_info->physical_device.physical_device_index = 0;
	p_info->physical_device.physical_device = p_info->physical_device.available_physical_device_array[p_info->physical_device.physical_device_index];
	p_info->physical_device.other_physical_device_index = 0;
	p_info->physical_device.other_physical_device = p_info->physical_device.available_physical_device_array[p_info->physical_device.other_physical_device_index];

	// 获取物理设备扩展列表
	p_info->physical_device.physical_device_extensions_count = 0;
	PFN_vkEnumerateDeviceExtensionProperties enumerate_device_extension_properties = nullptr;
	enumerate_device_extension_properties = (PFN_vkEnumerateDeviceExtensionProperties)p_info->library.vk_get_instance_proc_addr(p_info->instance.vk_instance, "vkEnumerateDeviceExtensionProperties");
	if (enumerate_device_extension_properties == nullptr) {
		PRINT_ERROR("load vkEnumerateDeviceExtensionProperties fail!\n");
		return false;
	}

	p_info->library.vk_result = enumerate_device_extension_properties(p_info->physical_device.physical_device, nullptr, &p_info->physical_device.physical_device_extensions_count, nullptr);
	if (p_info->library.vk_result != VK_SUCCESS || p_info->physical_device.physical_device_extensions_count == 0) {
		PRINT_ERROR("get extensions_device_count fail!\n");
		return false;
	}

	PFN_vkGetPhysicalDeviceFeatures get_physical_device_features = nullptr;
	get_physical_device_features = (PFN_vkGetPhysicalDeviceFeatures)p_info->library.vk_get_instance_proc_addr(p_info->instance.vk_instance, "vkGetPhysicalDeviceFeatures");
	...
	get_physical_device_properties = (PFN_vkGetPhysicalDeviceProperties)p_info->library.vk_get_instance_proc_addr(p_info->instance.vk_instance, "vkGetPhysicalDeviceProperties");
...
#ifdef DEBUG
	// 统计有几个物理设备
	for (uint32_t i = 0; i < p_info->physical_device.physical_device_count; i++) {
	...
	for (p_info->physical_device.physical_device_index = 0;
		...
		p_info->physical_device.physical_device = available_physical_device_array[p_info->physical_device.physical_device_index];
		get_physical_device_features(p_info->physical_device.physical_device, &p_info->physical_device.device_feature_array);
		get_physical_device_properties(p_info->physical_device.physical_device, &p_info->physical_device.device_properties);
	...
	PRINT_LOG(
		...
		p_info->physical_device.physical_device_index,
		p_info->physical_device.device_properties.deviceName);

	// 获取可用的物理设备扩展列表
	p_info->library.vk_result = enumerate_device_extension_properties(
		p_info->physical_device.physical_device, nullptr,
		&p_info->physical_device.physical_device_extensions_count, nullptr);
	if (p_info->library.vk_result != VK_SUCCESS || p_info->physical_device.physical_device_extensions_count == 0) {
		PRINT_ERROR("get extensions_device_count fail!\n");
		return false;
	}
	p_info->physical_device.available_physical_device_extension_array = (VkExtensionProperties *)cg_alloc_memory(
		p_info->p_memory_pool,
		p_info->physical_device.physical_device_extensions_count * sizeof(VkExtensionProperties));
	if (p_info->physical_device.available_physical_device_extension_array == nullptr) {
		...
		p_info->library.vk_result = enumerate_device_extension_properties(
			...
			&p_info->physical_device.available_physical_device_extension_array[0]);
		if (p_info->physical_device.physical_device_extensions_count == 0) {
	...
	// 打印可用的物理设备扩展的列表
	/* #ifdef DEBUG
		for (uint32_t i = 0; i < p_info->physical_device.physical_device_extensions_count; i++) {
			PRINT_LOG("available_physical_device_extension_array[%d] %s 版本%d;\n", i, (p_info->physical_device.available_physical_device_extension_array[i].extensionName), (p_info->physical_device.available_physical_device_extension_array[i]).specVersion);
		}
	#endif */
	...
	get_physical_device_memory_properties = (PFN_vkGetPhysicalDeviceMemoryProperties)p_info->library.vk_get_instance_proc_addr(p_info->instance.vk_instance, "vkGetPhysicalDeviceMemoryProperties");
	if (get_physical_device_memory_properties != nullptr) {
		get_physical_device_memory_properties(
			p_info->physical_device.physical_device, &p_info->physical_device.physical_device_memory_properties);
